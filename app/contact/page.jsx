...
const contacts = [
  {
    title: 'واتساب',
    description: 'راسلنا مباشرة للاستفسار السريع ومعرفة التفاصيل.',
    href: 'https://wa.me/201000000000',
    buttonLabel: 'افتح واتساب',
    icon: <WhatsappIcon />,
  },
  {
    title: 'تليفون',
    description: 'اتصل بنا مباشرة للتنسيق أو الاستفسار عن الأعمال.',
...
export const metadata = {
  title: 'تواصل معنا | المتخصص',
  description: 'تواصل مع المتخصص مباشرة عبر واتساب أو الهاتف أو فيسبوك.',
};
    ...
    <>
      <section className="border-b border-soft-divider bg-primary-background px-4 pb-10 pt-28 md:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <p className="font-body text-sm tracking-[0.2em] text-accent">تواصل معنا</p>
          <h1 className="mt-3 font-display text-3xl font-bold text-primary-text md:text-5xl">
            نحن قريبون منك بخطوة واحدة
          </h1>
          <p className="mt-6 max-w-2xl text-base leading-8 text-primary-text/80 md:text-lg">
            لا يوجد نموذج تواصل، لأننا نفضل أن تصل إلينا مباشرة بالطريقة الأنسب لك للحصول
            على رد أسرع وتجربة أبسط.
          </p>
        </div>
      </section>
      ...
      <section className="bg-primary-background px-4 py-16 md:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-2 xl:grid-cols-3">
          {contacts.map((item) => (
            <article
              key={item.title}
              className="rounded-[4px] border border-soft-divider bg-white p-8 shadow-sm"
            >
              <div className="mb-5 inline-flex h-14 w-14 items-center justify-center rounded-[4px] bg-primary-background text-accent">
                {item.icon}
              </div>
              <h2 className="font-display text-2xl font-bold text-primary-text">{item.title}</h2>
              <p className="mt-4 text-sm leading-7 text-primary-text/75">{item.description}</p>
              <a
                href={item.href}
                target={item.href.startsWith('http') ? '_blank' : undefined}
                rel={item.href.startsWith('http') ? 'noreferrer' : undefined}
                className="mt-6 inline-flex rounded-[4px] bg-accent px-5 py-3 text-sm font-bold text-white hover:bg-primary-text"
              >
                {item.buttonLabel}
              </a>
            </article>
